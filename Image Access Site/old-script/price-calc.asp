...
Sub ListPricingHV (sysid)
    ...
    select case kicsystem(sysid,0)
      ...
      case "ClickMini"
        if (kicsystem(sysid,2) = "Furniture") then 
          PricingData(sysid,30) = 5090     'On Stand Unit with furniture charge $289
        else
          if (kicsystem(sysid,2) = "Tabletop1") then 
...
Sub InsDiscPricing (sysid)
    ...
    select case kicsystem(sysid,0)
      ...
      case "ClickMini"
        if (kicsystem(sysid,2) = "Furniture") then 
          PricingData(sysid,30) = 4822     'On Stand Unit with furniture charge $273
        else
          if (kicsystem(sysid,2) = "Tabletop1") then 
            ...
            if (kicsystem(sysid,2) = "Tabletop2") then 
              ...
              PricingData(sysid,30) = 3829.68  'Tabletop3 -> Touch Only
            end if          
          end if           
        end if               
      case "Click"   
...
Sub InsDiscPricingHV (sysid)
    ...
    select case kicsystem(sysid,0)
      ...
      case "ClickMini"
        if (kicsystem(sysid,2) = "Furniture") then 
          PricingData(sysid,30) = 4581     'On Stand Unit with $260 furniture charge
        else
          if (kicsystem(sysid,2) = "Tabletop1") then 
