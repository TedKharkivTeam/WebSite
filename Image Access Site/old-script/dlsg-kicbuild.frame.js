...
var pricewrite = function(sys_id,sys_data) {
  ...
  set_summary_field(sys_id,'option','manager',sys_data[17]);
...
var sum_price = function(field) {
  var sum =   parseFloat($('#build-a-kic_1 '+field+' p.amount').html().replace(/[$,]+/g,""));
  sum = sum + parseFloat($('#build-a-kic_2 '+field+' p.amount').html().replace(/[$,]+/g,""));
  sum = sum + parseFloat($('#build-a-kic_3 '+field+' p.amount').html().replace(/[$,]+/g,""));
  sum = sum + parseFloat($('#build-a-kic_4 '+field+' p.amount').html().replace(/[$,]+/g,""));
  sum = sum + parseFloat($('#build-a-kic_5 '+field+' p.amount').html().replace(/[$,]+/g,""));
  if (isNaN(sum)) { sum = 0;};
  return sum;
}
...
var sort_list = function() {
  ...
  $('#kic_value .variables ul li').sortElements(function(a,b) {
    var a1 = parseFloat($(a).find('p.total').html());
    var b1 = parseFloat($(b).find('p.total').html());
    if (a1 == b1) {
      a1 = parseFloat($(a).find('p.device').html()) 
      b1 = parseFloat($(b).find('p.device').html())
    }
    return a1 > b1 ? -1 : 1;
  });
  var i = 0;
...
var build_bar = function(device,max_cost) {
  ...
  var total_cap = parseFloat($('#kic_value .variables input.annual.capacity').val().replace(/[$,]+/g,""));
  var device_annual_cap = parseFloat($('#kic_value .summary .'+device+' p.sublabel').html().replace(/[$,*\/(a-z)]+/g,""));
...
//here
  ...
  */
  ...
  $('#usage_slider').slider({ animate: true, min: 0, max: 100, step: 10, change: function(event, ui) { update_device_usage(); rebuild_value_chart(); }});
  $('#time_slider').slider({ animate: true, min: 0, max: 10, step: 1, change: function(event, ui) { labor_slider_update(); rebuild_value_chart(); }});
