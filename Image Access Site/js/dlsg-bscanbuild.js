...
var change_img_headings = function (sys_id) {
    ...
    else if ($('#' + sys_id + ' .features input.pc:checked').val()) {
        //single touch
        if (model == "BECM") {
...
var swap_option_images = function (sys_id) {
    ...
    else if ($('#' + sys_id + ' .features input.freeflow:checked').val()) {
...
var option_click_validate = function (sys_id, item_id) {
    ...
    else if ($(item_id).hasClass('freeflow')) {
...
var get_discount = function () {
    ...
    $.get(url, {DisCode: disc_code1}, function (data) {
        $('.disc_rate').val(data);
        build_pricing();
    });
...
var count_model_field = function (model, field) {
    ...
    if (model == $('#build-a-bscan_1 .models input:radio:checked').val()) {
        sum += parseInt($('#build-a-bscan_1 ' + field).val());
    }
    if (model == $('#build-a-bscan_2 .models input:radio:checked').val()) {
        sum += parseInt($('#build-a-bscan_2 ' + field).val());
    }
    if (model == $('#build-a-bscan_3 .models input:radio:checked').val()) {
        sum += parseInt($('#build-a-bscan_3 ' + field).val());
    }
    if (model == $('#build-a-bscan_4 .models input:radio:checked').val()) {
        sum += parseInt($('#build-a-bscan_4 ' + field).val());
    }
    if (model == $('#build-a-bscan_5 .models input:radio:checked').val()) {
        sum += parseInt($('#build-a-bscan_5 ' + field).val());
    }
    return sum;
};
...
$(document).ready(function () {
    ...
    $('.discount_amt span.sb').click(function () {
        $('.qty-disc-table').toggleClass('hide');
    });
    ...
    $('.qty-disc-table').click(function () {
        $('.qty-disc-table').addClass('hide');
    });
